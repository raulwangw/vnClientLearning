...
########################################################################
class NameCell1(QtGui.QTableWidgetItem):
    """用来显示合约中文名的单元格"""
        ...
        if self.dataKey:
            ...
            if key not in self.dataDict:
                ...
                for n, header in enumerate(self.headerList):
                    ...
                    if self.saveData:            # 如果设置了保存数据对象，则进行对象保存x
                        cell.data = data
...
class TradingWidget(QtGui.QFrame):
    ...
    def sendOrder(self):
        ...
        req.currency = currency
        req.productClass = productClass
        print("send Order %d"%self.mainEngine.sendOrder(req, gatewayName))
...
########################################################################
class basicPictureMonitor(QtGui.QWidget):
    
    signal = QtCore.pyqtSignal(type(Event()))
    
    def __init__(self,mainEngine,eventEngine,parent = None):
        super(basicPictureMonitor,self).__init__(parent)
        self.mainEngine = mainEngine
        self.eventEngine = eventEngine
    ...
    def initFrame(self):
        ...
        wd.widget = self
        wd.data = None
        wd.frameType = self.frameType
        self.mainEngine.registerFrame(wd)
        self.mainEngine.initFrame()
    ...
    def updateData(self,data):
        ...
        wd.data = data
        wd.frameType = self.frameType
        self.mainEngine.updateData(wd)
    
class dailyKPictureMonitor(basicPictureMonitor):
    def __init__(self,mainEngine,eventEngine,parent=None):
        super(dailyKPictureMonitor,self).__init__(mainEngine,eventEngine,parent)
        self.setFont(BASIC_FONT)
        self.setEvent(EVENT_DAILY_KLINE)
        self.setFrameType(FRAME_DAILY_KLINE)
        self.initFrame() 
        self.registerEvent()
        

########################################################################
class QTDMarketMonitor(BasicMonitor):
    def __init__(self,mainEngine,eventEngine,parent=None):
        """Constructor"""
        super(QTDMarketMonitor, self).__init__(mainEngine, eventEngine, parent)
        d = OrderedDict()
        d['stockid'] = {'chinese':u'股票代码', 'cellType':BasicCell}
        d['stockname'] = {'chinese':u'股票名称', 'cellType':BasicCell}
        d['newestprice'] = {'chinese':u'最新', 'cellType':BasicCell}
        d['uppecent'] = {'chinese':u'涨幅', 'cellType':BasicCell}
        d['downpecent'] = {'chinese':u'涨跌', 'cellType':BasicCell}
        d['upspeed'] = {'chinese':u'涨速', 'cellType':BasicCell}
        d['totalvolume'] = {'chinese':u'大单净量', 'cellType':BasicCell}
        d['totalhand'] = {'chinese':u'总手', 'cellType':BasicCell}
        d['changehand'] = {'chinese':u'换手', 'cellType':BasicCell}
        d['volumerate'] = {'chinese':u'量比', 'cellType':BasicCell}
        d['nowhand'] = {'chinese':u'现手', 'cellType':BasicCell}
        d['openprice'] = {'chinese':u'开盘', 'cellType':BasicCell}
        d['yesterdayprice'] = {'chinese':u'作收', 'cellType':BasicCell}
        d['highestprice'] = {'chinese':u'最高', 'cellType':BasicCell}
        d['lowestprice'] = {'chinese':u'最低', 'cellType':BasicCell}
        d['PE'] = {'chinese':u'市盈(动)', 'cellType':BasicCell}
        d['PB'] = {'chinese':u'市净率', 'cellType':BasicCell}
        d['weibi'] = {'chinese':u'委比', 'cellType':BasicCell}
        d['swing'] = {'chinese':u'振幅', 'cellType':BasicCell}
        d['totalmarketvalue'] = {'chinese':u'总市值', 'cellType':BasicCell}

        self.setHeaderDict(d)
        
        self.setDataKey('qtdMarketID')
        self.setEventType(EVENT_MD)
        self.setFont(BASIC_FONT)
        self.initTable()
        self.registerEvent()

########################################################################
class QTDStockSelectStratistic(BasicMonitor):
    def __init__(self,mainEngine,eventEngine,parent=None):
        """Constructor"""
        super(QTDStockSelectStratistic, self).__init__(mainEngine, eventEngine, parent)
        d = OrderedDict()
        d['stockid'] = {'chinese':u'股票代码', 'cellType':BasicCell}
        d['curprice'] = {'chinese':u'股票价格', 'cellType':BasicCell}
        d['uppercent'] = {'chinese':u'涨幅', 'cellType':BasicCell}
        self.setHeaderDict(d)
        
        self.setDataKey('qtdStockSelectStratistic')
        self.setEventType(EVENT_SSELSTRATIS)
        self.setFont(BASIC_FONT)
        self.initTable()
        self.registerEvent()


class QTDCommissionMonitor(BasicMonitor):
    def __init__(self,mainEngine,eventEngine,parent=None):
        """Constructor"""
        super(QTDCommissionMonitor, self).__init__(mainEngine, eventEngine, parent)
        d = OrderedDict()
        d['commissiontime'] = {'chinese':u'委托时间', 'cellType':BasicCell}
        d['commissionavg'] = {'chinese':u'委托/均价', 'cellType':BasicCell}
        d['commissiontrade'] = {'chinese':u'委托/成交', 'cellType':BasicCell}
        d['commissionstate'] = {'chinese':u'状态', 'cellType':BasicCell}

        self.setHeaderDict(d)
        
        self.setDataKey('qtdCommissionID')
        self.setEventType(EVENT_COMMISSION)
        self.setFont(BASIC_FONT)
        self.initTable()
        self.registerEvent()

class QTDTransactionMonitor(BasicMonitor):
    def __init__(self,mainEngine,eventEngine,parent=None):
        """Constructor"""
        super(QTDTransactionMonitor, self).__init__(mainEngine, eventEngine, parent)
        d = OrderedDict()
        d['transactiontime'] = {'chinese':u'成交时间', 'cellType':BasicCell}
        d['transactionprice'] = {'chinese':u'成交价', 'cellType':BasicCell}
        d['transactionquntity'] = {'chinese':u'成交量', 'cellType':BasicCell}
        d['transactionvalue'] = {'chinese':u'成交额度', 'cellType':BasicCell}

        self.setHeaderDict(d)
        
        self.setDataKey('qtdtransactionID')
        self.setEventType(EVENT_TRADE)
        self.setFont(BASIC_FONT)
        self.initTable()
        self.registerEvent()

class QTDHoldStockMonitor(BasicMonitor):
    def __init__(self,mainEngine,eventEngine,parent=None):
        """Constructor"""
        super(QTDHoldStockMonitor, self).__init__(mainEngine, eventEngine, parent)
        d = OrderedDict()
        d['stockid'] = {'chinese':u'股票代码', 'cellType':BasicCell}
        d['trade'] = {'chinese':u'买卖', 'cellType':BasicCell}
        d['costprice'] = {'chinese':u'成本价', 'cellType':BasicCell}
        d['holdstocknumber'] = {'chinese':u'持仓数量', 'cellType':BasicCell}
        d['usefulnumber'] = {'chinese':u'可用数量', 'cellType':BasicCell}
        d['hangnumber'] = {'chinese':u'挂盘数量', 'cellType':BasicCell}
        d['floatingpl'] = {'chinese':u'浮动盈亏', 'cellType':BasicCell}

        self.setHeaderDict(d)
        
        self.setDataKey('qtdholdstockID')
        self.setEventType(EVENT_POSITION)
        self.setFont(BASIC_FONT)
        self.initTable()
        self.registerEvent()
        
class QTDLogMonitor(BasicMonitor):
    def __init__(self,mainEngine,eventEngine,parent=None):
        """Constructor"""
        super(QTDLogMonitor, self).__init__(mainEngine, eventEngine, parent)
        d = OrderedDict()
        d['logtime'] = {'chinese':u'时间', 'cellType':BasicCell}
        d['logtype'] = {'chinese':u'日志类型', 'cellType':BasicCell}
        d['logsystem'] = {'chinese':u'系统信息', 'cellType':BasicCell}

        self.setHeaderDict(d)
        
        self.setDataKey('eQtdLogID')
        self.setEventType(EVENT_LOG)
        self.setFont(BASIC_FONT)
        self.initTable()
        self.registerEvent()

class QTDCapitalMonitor(BasicMonitor):
    def __init__(self,mainEngine,eventEngine,parent=None):
        """Constructor"""
        super(QTDCapitalMonitor, self).__init__(mainEngine, eventEngine, parent)
        d = OrderedDict()
        d['capital'] = {'chinese':u'总资产', 'cellType':BasicCell}
        d['floatingpl'] = {'chinese':u'浮动盈亏', 'cellType':BasicCell}
        d['logstotalmarketvalueystem'] = {'chinese':u'总市值', 'cellType':BasicCell}
        d['capablecapital'] = {'chinese':u'可取', 'cellType':BasicCell}
        d['userfulcapital'] = {'chinese':u'可用', 'cellType':BasicCell}

        self.setHeaderDict(d)
        
        self.setDataKey('qtdcapitalID')
        self.setEventType(EVENT_CAPITAL) 
        self.setFont(BASIC_FONT)
        self.initTable()
        self.registerEvent()
