...
class MainEngine(object):
    ...
    def __init__(self):
        ...
        self.pictureEngine = PictureEngine(self.eventEngine)
        # MongoDB数据库相关
    ...
    def initGateway(self):
        ...
        try:
            from qtdGateway.qtdGateway import QtdGateway
            self.addGateway(QtdGateway, 'QTD')
            self.gatewayDict['QTD'].setQryEnabled(True)
        except Exception, e:         
            print(e)
...
########### QTD Function####################
    def registerFrame(self,widgetData):
        frameType = widgetData.frameType
        if frameType == None:
            return
        if self.frameDict.has_key(frameType):
            widgetEngine = self.frameDict[frameType]
            widgetEngine.setWidgetData(widgetData)
        else:
            if frameType == FRAME_DAILY_KLINE:
                self.frameDict[frameType] = PictureDaily_KLineEngine(widgetData)
    
    def initFrame(self):
        for frameType in self.frameDict:
            self.frameDict[frameType].initFrame()
    ...
    def updateData(self,widgetData):
        ...
        if frameType == None:
            return
        if self.frameDict.has_key(frameType):
            self.frameDict[frameType].updateData(widgetData)
